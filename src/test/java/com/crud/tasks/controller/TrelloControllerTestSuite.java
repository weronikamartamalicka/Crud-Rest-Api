...
class TrelloControllerTestSuite {
    ...
    void shouldFetchTrelloBoards() throws Exception {
        when(trelloFacade.fetchTrelloBoards()).thenReturn(List.of(new TrelloBoardDto(
                "Test task", "1", List.of(new TrelloListDto(
                        "1", "Test list", false)))));

        mockMvc
                .perform(MockMvcRequestBuilders.get("/v1/trello/boards").contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk())
